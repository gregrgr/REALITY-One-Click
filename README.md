...
- `panel/`：FastAPI 管理后端、SQLite 数据层、Clash/VLESS 订阅生成、Xray 配置生成、CLI。
- `templates/`：Nginx HTTP/HTTPS/stream 和 systemd 模板。
- `tests/`：核心配置和订阅生成测试。
- `docs/`：配置、API、排障文档。
...
- VPS 安全组/防火墙放行 `80/tcp` 和 `443/tcp`。
...
- 提供管理后端，用于查看服务状态、管理用户、重启 Xray、复制订阅地址。
...
- HTTPS 管理面板，例如 `https://panel.example.com`。
- HTTPS 订阅接口，例如 `https://panel.example.com/sub/<token>/clash.yaml`。
- ACME 自动签发和续期证书。

推荐准备一个独立域名或子域名指向 VPS：

- `panel.example.com`：管理后端和订阅接口。
- Xray REALITY 客户端连接地址可以使用服务器 IP 或用户输入的节点域名。

### 端口设计

默认推荐使用 443 对外提供 REALITY，同时让管理面板也支持 HTTPS。由于二者都需要 443，本项目采用 Nginx `stream` SNI 分流：

- `0.0.0.0:443`：Nginx stream 入口。
- SNI 等于面板域名时，转发到本机 HTTPS 后端 `127.0.0.1:8443`。
- 其他 SNI 默认转发到 Xray REALITY `127.0.0.1:1443`。
...
Client
  ...
  v
Nginx stream
  |-- SNI = panel.example.com --> Nginx HTTPS 127.0.0.1:8443 --> Admin API 127.0.0.1:8080
  |
  `-- default -----------------> Xray REALITY 127.0.0.1:1443
...
- Nginx：HTTP 站点、HTTPS 反代、stream SNI 分流。
...
- 管理员密码，留空则自动生成强密码。
- Xray 对外端口，默认 `443`。
- REALITY 监听地址，默认 `127.0.0.1:1443`。
...
5. 申请 HTTPS 证书。
6. 渲染 Nginx HTTP、HTTPS、stream 配置。
...
```text
https://panel.example.com/sub/<token>/clash.yaml
https://panel.example.com/sub/<token>/vless.txt
```
...
/etc/nginx/stream-conf.d/reality.conf
/opt/proxy-panel/
...
- 管理后端是否需要完整 Web UI，还是首版先提供 API + 简单 HTML 页面。
- 是否要求多节点订阅，还是当前 VPS 单节点订阅即可。
- 是否需要流量统计和用户限速。
- 是否需要支持 Cloudflare DNS API 自动签发证书。
- 是否需要 Docker 部署模式。
