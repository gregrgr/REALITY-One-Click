...
```bash
git clone https://github.com/gregrgr/REALITY-One-Click.git proxy-panel
cd proxy-panel
bash install.sh
```
...
管理后端用户卡片同时展示 Clash 订阅链接与 VLESS URI 的 SVG 二维码,扫码即可在手机端 Mihomo/Clash Meta、v2rayN、Nikki 等客户端导入。二维码在服务端用 `segno` 离线生成,不依赖任何第三方 CDN。
...
- 强制代理规则：Claude/Anthropic、ChatGPT/OpenAI、HuggingFace、Figma、GitHub、Twitter/X、YouTube、Notion、Discord、Reddit、Telegram 网页/登录入口等域名始终走 `Proxy`，优先级高于国内直连规则；`Proxy` 和 `Final` 分组不提供 `DIRECT` 选项。
...
- IP 直连防绕过：启用 Mihomo `sniffer`，对 443/8443 TLS、80/8080 HTTP、443 QUIC 嗅探目标域名，避免硬编码 IP 的应用绕开域名规则。
...
- 登录 / 退出。
- 查看 Xray、Nginx、面板服务状态,出口路径 / 延迟 / 出口 IP 实时探测。
- 查看和编辑当前节点参数。
- 查看每个用户上传、下载、总流量。
- 新增、禁用、删除 VLESS 用户。
- 重置用户 UUID 和订阅 token。
- 每个用户独立展示 Clash 订阅链接和 VLESS URI,提供一键复制和**离线二维码**(SVG,扫码即可在 Mihomo/Clash Meta、v2rayN、Nikki 等客户端导入)。
