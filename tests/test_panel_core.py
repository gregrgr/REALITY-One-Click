...
class PanelCoreTest(unittest.TestCase):
    def test_database_user_and_subscriptions(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("DOMAIN-SUFFIX,github.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,githubusercontent.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,twitter.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,x.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,youtube.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,googlevideo.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,notion.so,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,discord.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,reddit.com,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,huggingface.co,Proxy", clash["rules"])
            self.assertIn("DOMAIN-SUFFIX,telegram.org,Proxy", clash["rules"])
            ...
            self.assertFalse(
                any(group["name"] == "Local" for group in clash["proxy-groups"]),
                "Local proxy-group was unused and should be removed",
            )
            self.assertEqual(clash["find-process-mode"], "off")
            self.assertTrue(clash["sniffer"]["enable"])
            self.assertTrue(clash["sniffer"]["parse-pure-ip"])
            self.assertIn(443, clash["sniffer"]["sniff"]["TLS"]["ports"])
            self.assertIn("#Proxy", clash["dns"]["nameserver-policy"]["+.github.com"][0])
            self.assertIn("#Proxy", clash["dns"]["nameserver-policy"]["+.youtube.com"][0])
            self.assertIn("+.github.com", clash["dns"]["fallback-filter"]["domain"])
            self.assertIn("+.youtube.com", clash["dns"]["fallback-filter"]["domain"])
